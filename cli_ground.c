...
void get_config_params(char* filepath){
	...
	if(!config_lookup_string(&conf, "main.cmd_log", &tmpstr)) {
		printf("Missing main.cmd_log in %s\n",filepath);
		config_destroy(&conf);
		exit(0);
	}
	
	gs_conf.cmd_log = strdup(tmpstr);
...
int send_packet(Packet pkt){
	...
	snprintf(buf,MAXLINE,"%d\n%d\n%d\n%s\n%d\n%s\n%d\n%d\n%d\n",pkt.start, pkt.cmd_primary, pkt.utc, small_buf, pkt.num_data, big_buf, pkt.num_bigpack, pkt.checksum, pkt.end);
...
// 40-second countdown timer for RFSOC boot-up
void rfsoc_boot_timer() {
    printf("\n"  "Please wait 40 seconds for complete RFSoC bootup before starting spectrometer..." RESET "\n");
    
    for (int i = 40; i > 0; i--) {
        printf("\r" BRIGHT_CYAN "Bootup countdown: " BRIGHT_WHITE "%2d seconds remaining..." RESET, i);
        fflush(stdout);
        sleep(1);
    }
    
    printf("\r" BRIGHT_GREEN "RFSoC bootup complete!                    " RESET "\n\n");
}

// 60-second countdown timer for backend startup
void backend_startup_timer() {
    printf("\n" "Please wait 60 seconds for complete backend startup..." RESET "\n");
    
    for (int i = 60; i > 0; i--) {
        printf("\r" "Backend startup countdown: " "%2d seconds remaining..." RESET, i);
        fflush(stdout);
        sleep(1);
    }
    
    printf("\r" "Backend startup complete!                      " RESET "\n\n");
}

// Help function to display all available commands
void print_help() {
    printf("\n" BRIGHT_CYAN "===== BCP Ground Station Commands =====" RESET "\n\n");
    
    printf(BRIGHT_YELLOW "System Commands:" RESET "\n");
    printf("  test_Oph                - Test connection to Ophiuchus\n");
    printf("  test_Sag                - Test connection to Sagittarius\n");
    printf("  reset_counter_Oph       - Reset Ophiuchus command counter\n");
    printf("  reset_counter_Sag       - Reset Sagittarius command counter\n");
    printf("  exit                    - Exit ground station\n");
    printf("  exit_bcp                - Exit BCP on both computers\n");
    printf("  help                    - Show this help message\n\n");
    
    printf(BRIGHT_YELLOW "Telescope Commands (Ophiuchus):" RESET "\n");
    printf("  lock_tel_cmd            - Lock telescope\n");
    printf("  unlock_tel_cmd          - Unlock telescope\n");
    printf("  reset_lock              - Reset telescope lock\n");
    printf("  lockpin_start           - Start lock pin\n");
    printf("  lockpin_stop            - Stop lock pin\n");
    printf("  motor_stop              - Stop motor\n");
    printf("  motor_start             - Start motor\n");
    printf("  gotoenc                 - Go to encoder position\n");
    printf("  encdither               - Encoder dither\n");
    printf("  enctrack                - Encoder track\n");
    printf("  enconoff                - Encoder on/off\n");
    printf("  trackdither             - Track dither\n");
    printf("  stop_scan               - Stop scan\n");
    printf("  set_offset_az           - Set azimuth offset\n");
    printf("  set_offset_el           - Set elevation offset\n");
    printf("  park                    - Park telescope\n\n");
    
    printf(BRIGHT_YELLOW "Camera Commands (Ophiuchus):" RESET "\n");
    printf("  bvexcam_solve_stop      - Stop camera solve\n");
    printf("  bvexcam_solve_start     - Start camera solve\n");
    printf("  bvexcam_save_stop       - Stop camera save\n");
    printf("  bvexcam_save_start      - Start camera save\n");
    printf("  bvexcam_set_exp         - Set camera exposure\n");
    printf("  focus_bvexcam           - Focus camera\n");
    printf("  bvexcam_start           - Start camera\n");
    printf("  bvexcam_stop            - Stop camera\n\n");
    
    printf(BRIGHT_YELLOW "Receiver Commands (Sagittarius):" RESET "\n");
    printf("  receiver_start          - Start receiver\n");
    printf("  receiver_stop           - Stop receiver\n");
    printf("  stop_gps                - Stop GPS\n");
    printf("  start_gps               - Start GPS\n");
    printf("  stop_spec               - Stop spectrometer\n");
    printf("  start_spec              - Start spectrometer\n");
    printf("  start_spec_120kHz       - Start 120kHz spectrometer\n");
    printf("  stop_spec_120kHz        - Stop 120kHz spectrometer\n\n");
    
    printf(BRIGHT_YELLOW "RFSoC Commands (Sagittarius):" RESET "\n");
    printf("  "  "rfsoc_on" RESET "                - Turn on RFSoC (includes 40s boot timer)\n");
    printf("  rfsoc_off               - Turn off RFSoC\n");
    printf("  rfsoc_configure_ocxo    - Configure RFSoC OCXO\n\n");
    
    printf(BRIGHT_YELLOW "VLBI Commands (Sagittarius):" RESET "\n");
    printf("  start_vlbi              - Start VLBI\n");
    printf("  stop_vlbi               - Stop VLBI\n");
    printf("  start_backend           - Start backend (includes 60s startup timer)\n");
    printf("  stop_backend            - Stop backend\n\n");
    
    printf(BRIGHT_YELLOW "Timing Commands (Sagittarius):" RESET "\n");
    printf("  start_timing_chain      - Start timing chain\n");
    printf("  stop_timing_chain       - Stop timing chain\n");
    printf("  start_ticc              - Start TICC\n");
    printf("  stop_ticc               - Stop TICC\n\n");
    
    printf(BRIGHT_YELLOW "Heater Commands (Sagittarius):" RESET "\n");
    printf("  start_heater_box        - Start heater box\n");
    printf("  start_heaters           - Start heaters\n");
    printf("  stop_heaters            - Stop heaters\n");
    printf("  start_pr59              - Start PR59\n");
    printf("  stop_pr59               - Stop PR59\n\n");
    
    printf(BRIGHT_YELLOW "Position Commands (Sagittarius):" RESET "\n");
    printf("  start_position_box      - Start position box\n");
    printf("  stop_position_box       - Stop position box\n");
    printf("  position_box_on         - Turn position box on\n");
    printf("  position_box_off        - Turn position box off\n\n");
...
void create_packet(Packet* pkt, const uint8_t cmd1, const int16_t* data, const uint8_t small_len, const double_t* bigpack, const uint8_t big_len, const uint8_t dest) {
    ...
    const size_t base_size = 8+ small_len * sizeof(int16_t) + big_len * sizeof(double_t); // fields before checksum
...
void exec_command(char* input) {
    ...
    scan = sscanf(input, "%s %[^\t\n]", cmd, arg);
    fprintf(cmd_log,"[%ld]%s",time(NULL),input);
	...
	create_packet(&pkt, com, payload, 0, big_payload, 0, SAG);
    	...
    	close(sock_Oph);
    	return;
    } else if (strcmp(cmd, "reset_counter_Oph")==0){
        count_Oph = 0;
        return;
    }else if (strcmp(cmd, "reset_counter_Sag")==0){
        count_Sag = 0;
        return;
    ...
    }else if (strcmp(cmd, "bvexcam_set_focus_inf") == 0) {
        com = bvexcam_set_focus_inf;
        
        create_packet(&pkt, com, payload, 0, big_payload, 0, OPH);
        

    }else if (strcmp(cmd, "bvexcam_set_focus") == 0) {
        com = bvexcam_set_focus;
        payload = (int16_t*)malloc(sizeof(int16_t));
        ret = sscanf(arg,"%hd",&payload[0]);
        if(ret != 1){
            printf("Invalid format: Usage is bvexcam_set_focus <focus_pos> where focus_pos is the focus position\n");
            return;
        }
        create_packet(&pkt, com, payload, 1, big_payload, 0, OPH);
    ...
    }else if(strcmp(cmd,"trackdither")==0){
    	big_payload = (double_t *)malloc(4*sizeof(double_t));
    	payload = (int16_t *)malloc(sizeof(int16_t));
    	ret = sscanf(arg,"%lf,%lf,%lf,%lf,%hd",&big_payload[0],&big_payload[1],&big_payload[2],&big_payload[3],&payload[0]);
	if(ret !=5){
		printf("Invalid format, usage is trackdither <ra>,<dec>,<scanlen>,<scan_vel>,<nscans> where ra is the target right ascension (0,360) in degrees, dec is the declination (-90,90) in degrees, scan_len is the scan_length (0,10), scan_vel is the scan speed in degrees per second (0.5,3) and nscans is the number of scans\n");
		return;
	}
	if((big_payload[0] < 0) || (big_payload[0]>360)){
                printf("Invalid right ascension should be between 0 and 360 degrees\n");
	...
	if((big_payload[2] < 0) || (big_payload[2] > 10)){
		printf("Invalid scan length, should be between 0 and 10 degrees\n");
		return;
	}
	if((big_payload[3]<0.5) || (big_payload[3]>3)){
                printf("Invalid scan speed should be between 0.5 and 3 degrees per second\n");
                return;
        }
	if(&payload[0]<=0){
    	...
    	com = trackdither;
    	create_packet(&pkt, com, payload, 1, big_payload, 4, OPH);
	...
	com = receiver_stop;
    	...
    	ret = sscanf(arg,"%hd",&payload[0]);
    	if(ret != 1){
    	    printf("Invalid format usage is: start_vlbi <ssd_id> where ssd_id is 1 or 2\n");
            return;
    	}
    	if(payload[0]!=1 && payload[0] !=2){
    	    printf("Invalid hard drive id should be 1 or 2\n");
    	    return;
    	}
    ...
    if(!cmd_unknown){
    	if(confirm_send()){
    		...
    		int previous_count_Sag = count_Sag;
    		int send_result = send_packet(pkt);
    		
    		// Check if this was an rfsoc_on command and it was successfully received by Sagittarius
    		if (com == rfsoc_on && send_result == 1 && count_Sag > previous_count_Sag) {
    			printf("RFSOC_ON command confirmed by Sagittarius. Starting boot timer...\n");
    			rfsoc_boot_timer();
    		}
    		// Check if this was a start_backend command and it was successfully received by Sagittarius
    		else if (com == start_backend && send_result == 1 && count_Sag > previous_count_Sag) {
    			printf("START_BACKEND command confirmed by Sagittarius. Starting backend timer...\n");
    			backend_startup_timer();
    		}
    	}else{
    		printf("Not sending command\n");
    	}
